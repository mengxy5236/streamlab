...
- JPA + MyBatis
- Redis
- RabbitMQ for video transcoding only
- Aliyun OSS
- FFmpeg / ffprobe
...
- Video transcoding is the only RabbitMQ-backed asynchronous pipeline.
- Redis is used for counters, caching, progress state, and lightweight distributed coordination.
- Scheduled jobs sync buffered view and like counters from Redis back into MySQL.
